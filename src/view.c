...
static void rewrite_conversation_file(const char* user1, const char* user2, const Message conversation[], int count) {
    ...
    if (fp) {
        ...
        printf(COLOR_RED "Error: Could not rewrite conversation file for '%s' and '%s'.\n" COLOR_RESET, user1, user2);
    }
}

// Lists all registered users
void list_users() {
     printf(COLOR_CYAN "--- Registered Users ---\n" COLOR_RESET);
...
void check_notification() {
    printf(COLOR_CYAN "--- Notifications for %s ---\n" COLOR_RESET, current_username);
    ...
    for (int i = 0; i < partner_count; i++) {
        ...
        for (int j = 0; j < conv_count; j++) {
            if (strcmp(conversation_buffer[j].recipient, current_username) == 0 && conversation_buffer[j].read_status == 0) {
                ...
                printf(COLOR_YELLOW "From %s " COLOR_RESET "at %s: " TEXT_BOLD "%s\n" COLOR_RESET, 
    ...
    if (!found_new_messages) {
        printf(COLOR_YELLOW "No new notifications.\n" COLOR_RESET);
    }
    printf(COLOR_CYAN "---------------------------\n" COLOR_RESET);
}
...
void check_conve(const char* conv_name) {
    if (find_user_index(conv_name) == -1) {
        printf(COLOR_RED "Error: User '%s' does not exist.\n" COLOR_RESET, conv_name);
        return;
    }
    
    printf(COLOR_CYAN "--- Conversation with %s ---\n" COLOR_RESET, conv_name);
    Message conversation_buffer[MAX_MESSAGES];
    int conv_count = 0;
    ...
    if (conv_count > 0) {
        for (int i = 0; i < conv_count; i++) {
            ...
            // Color own messages differently from the other person's messages
            if (strcmp(conversation_buffer[i].sender, current_username) == 0) {
                 printf(COLOR_GREEN "[%s] You: %s\n" COLOR_RESET, time_str, conversation_buffer[i].content);
            } else {
                 printf(COLOR_YELLOW "[%s] %s: %s\n" COLOR_RESET, time_str, conversation_buffer[i].sender, conversation_buffer[i].content);
            }
        }
    } else {
        printf(COLOR_YELLOW "No conversation history with %s.\n" COLOR_RESET, conv_name);
    }
    printf(COLOR_CYAN "----------------------------\n" COLOR_RESET);
}
...
void list_conv() {
    printf(COLOR_CYAN "--- Conversations for %s ---\n" COLOR_RESET, current_username);
    char partners[MAX_USERS][MAX_USERNAME_LEN + 1];
    int partner_count = 0;
    ...
    if (partner_count > 0) {
        ...
        printf(COLOR_YELLOW "No active conversations.\n" COLOR_RESET);
    }
    printf(COLOR_CYAN "----------------------------\n" COLOR_RESET);
}